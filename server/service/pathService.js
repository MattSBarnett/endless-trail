const geocodeLocation = async(name) => {    
  const result = await fetch(`https://nominatim.openstreetmap.org/search?q=${name}&format=json&countrycodes=gb&limit=1`,
...
const getPathBetweenCoordinates = async (startCoord, endCoord) => {
  const res = await fetch(`https://api.openrouteservice.org/v2/directions/foot-hiking?api_key=${process.env.OPEN_ROUTE_SERVICE_API_KEY}&start=${startCoord.lon},${startCoord.lat}&end=${endCoord.lon},${endCoord.lat}`)
